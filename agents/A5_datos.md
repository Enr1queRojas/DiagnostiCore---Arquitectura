...
`blackboard/contracts/{client_id}_contract.json`, sección `A5_datos`.
...
Desde la perspectiva de datos, el Excel Sagrado es la evidencia más directa de ausencia de gobernanza. El dato crítico vive fuera del sistema oficial — en hojas de cálculo personales de empleados de la organización, frecuentemente en laptops individuales o servidores ad-hoc sin backup sistemático — porque el sistema no lo captura, no lo captura bien, o no es accesible para quien lo necesita. El agente identifica: ¿qué dato crítico vive en Excel interno que debería estar en el sistema? ¿Por qué no está en el sistema?

**Criterio de exclusión crítico:** el patrón P1 NO aplica cuando los Excels son producidos por externos (despachos contables, consultores financieros, asesores) bajo sus propios procesos. Esos archivos son entregables de terceros, no Shadow IT interno. Clasificarlos como P1 produciría una intervención de gobernanza de datos donde la intervención real es de dependencia analítica externa (que puede manifestarse como P2 — Director Orquesta si el conocimiento vive en el externo, o como hallazgo de cultura analítica). Cuando los Excels relevantes son externos, el agente documenta la dependencia en los hallazgos correspondientes sin forzar la etiqueta P1.
...
**Consistencia numérica cruzada entre secciones:** toda cifra financiera que aparezca en más de una sección del documento debe ser numéricamente idéntica y derivable del mismo cálculo base. Antes de cerrar el documento, el agente verifica explícitamente que los números del Resumen Ejecutivo son matemáticamente consistentes con los cálculos de Sección 09. Si hay discrepancia entre secciones, la sección que se corrige es siempre el Resumen Ejecutivo — la fuente de verdad del cálculo es Sección 09. Dos versiones de la misma cifra en el mismo documento es un error metodológico que invalida el análisis cuantitativo.

**Atribución obligatoria de supuestos cuantitativos:** todo supuesto numérico utilizado en un cálculo de impacto debe declarar explícitamente su fuente o su método de inferencia. Si el supuesto es inferido (no declarado directamente por una fuente), se etiqueta como "supuesto de trabajo" y el resultado se marca como "sujeto a validación." Supuestos sin atribución no pueden producir cifras absolutas en el Resumen Ejecutivo ni en palancas de intervención. El formato correcto cuando el supuesto es inferido: "Impacto estimado = [variable] × [diferencial] × [factor conservador]. [Variable] inferido de [fuente A] ÷ [fuente B] = [valor]. Cifra sujeta a validación — no usar en entregables cliente hasta confirmar." Si el supuesto no puede atribuirse a ninguna fuente disponible, el cálculo de impacto se omite y se incluye como hipótesis pendiente.

**Consistencia entre nivel de confianza y score condicional:** el nivel de confianza declarado debe ser coherente con el estado de las hipótesis pendientes de alta urgencia. Regla específica: si existe al menos una hipótesis de urgencia Crítica o Alta cuya confirmación podría modificar el score en 0.2 puntos o más, el nivel de confianza máximo permitido es Medio. La asignación de Medio-Alto o Alto requiere que ninguna hipótesis pendiente de urgencia Crítica o Alta tenga el potencial de mover el score 0.2 puntos o más. Cuando el nivel de confianza se limita por hipótesis pendiente, el Resumen Ejecutivo debe incluir: "Score sujeto a revisión [dirección] pendiente validación [H-número]: [descripción de la hipótesis y rango de ajuste posible]."
...
14. **Consistencia numérica cruzada:** antes de cerrar el documento, verificar que toda cifra financiera que aparezca en más de una sección es matemáticamente idéntica. Si hay discrepancia, corregir el Resumen Ejecutivo para que sea consistente con Sección 09. Dos versiones del mismo número en el mismo documento es un error metodológico que invalida el análisis cuantitativo.
15. **Atribución de supuestos cuantitativos:** todo número usado en un cálculo de impacto que no proviene directamente de una fuente citada debe declararse como "supuesto de trabajo" con el método de inferencia explícito. Supuesto sin atribución = cifra no reportable en entregables. Si el supuesto no puede atribuirse, reemplazar la cifra absoluta por la fórmula abierta con la variable sin resolver.
16. **Coherencia entre confianza y score condicional:** si alguna hipótesis de urgencia Crítica o Alta puede mover el score 0.2 puntos o más, el nivel de confianza máximo es Medio. Confianza Medio-Alto o Alto solo cuando ninguna hipótesis pendiente de urgencia Crítica o Alta tenga ese potencial de movimiento. Cuando el nivel se limita por hipótesis, documentar en el Resumen Ejecutivo: "Score sujeto a revisión [dirección] pendiente [H-N]."

---

*InnoVerse DiagnostiCore — Sistema de Diagnóstico 360*
*Agente 05 — Datos, Analítica e BI*
*Versión 2.0 | Marzo 2026 | Uso interno exclusivo — Confidencial*

**Changelog v2.0:**
- Corrección de definición de Patrón 1 (El Excel Sagrado): se agrega criterio de exclusión explícito para Excels producidos por externos. Clasificar análisis financieros de terceros como P1 producía intervención de gobernanza de datos donde la intervención real era de dependencia analítica externa.
- Regla 14 nueva — Consistencia numérica cruzada: toda cifra financiera en más de una sección debe ser matemáticamente idéntica. La fuente de verdad del cálculo es Sección 09; el Resumen Ejecutivo se corrige ante discrepancia.
- Regla 15 nueva — Atribución obligatoria de supuestos cuantitativos: todo número inferido (no citado directamente de fuente) se etiqueta como "supuesto de trabajo" y el resultado como "sujeto a validación". Supuesto sin atribución = cifra no reportable en entregables cliente.
- Regla 16 nueva — Coherencia entre nivel de confianza y score condicional: hipótesis de urgencia Crítica o Alta con potencial de mover score 0.2+ puntos limita el nivel de confianza máximo a Medio. Se agrega instrucción de nota condicional en Resumen Ejecutivo.
- Estándares de calidad: se incorporan las tres reglas anteriores como estándares de calidad adicionales con descripción de aplicación práctica.
- Origen: gaps identificados durante diagnóstico de La Gretta (marzo 2026), caso de prueba del Agente 05 v1.0.
